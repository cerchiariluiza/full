...
def context_security(app:Flask):
    ...
    def create_user():
        with app.app_context():
            db.create_all(bind=['regex_cartoes'])
            if db.session.query(User).filter_by(username='master',email='teste@teste.com.br').count() == 0:
                user_datastore.create_user(username='master', email='teste@teste.com.br', password=encrypt_password('12345'))
            db.session.commit()
            user_datastore.find_or_create_role(name='estagiario', description='Estágiario')
            user_datastore.find_or_create_role(name='admin', description='Permissao total')
            user_datastore.add_role_to_user('teste@teste.com.br', 'admin')
            db.session.commit()
