...
class Config:
    ...
    SQLALCHEMY_DATABASE_URI2 = 'mysql+pymysql://%(user)s:%(pw)s@%(host)s:%(port)s/%(db)s' % DATABASE2
    SQLALCHEMY_DATABASE_URI3 = 'mysql+pymysql://%(user)s:%(pw)s@%(host)s:%(port)s/%(db)s' % DATABASE3
    SQLALCHEMY_BINDS = {
        'regex_cartoes':         SQLALCHEMY_DATABASE_URI2,
        'temp':            SQLALCHEMY_DATABASE_URI3
